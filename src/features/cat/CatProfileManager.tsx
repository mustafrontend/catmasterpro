          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">{t('profile.languageSub', '12 Dil Desteği')}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-6">
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/60">
                <div className="flex items-center text-xs font-semibold text-slate-400 gap-1.5 mb-1">
                  <Calendar className="w-3.5 h-3.5 text-sky-500" /> {t('profile.age', 'Yaş')}
                </div>
                <p className="text-sm font-extrabold text-slate-900">{activeCat.ageYears} {t('profile.years', 'Yıl')} {activeCat.ageMonths} {t('profile.months', 'Ay')}</p>
              </div>

              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/60">
                <div className="flex items-center text-xs font-semibold text-slate-400 gap-1.5 mb-1">
                  <Weight className="w-3.5 h-3.5 text-emerald-500" /> {t('profile.weight', 'Kilo')}
                </div>
              ...
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/60">
                <div className="flex items-center text-xs font-semibold text-slate-400 gap-1.5 mb-1">
                  <ShieldCheck className="w-3.5 h-3.5 text-amber-500" /> {t('profile.microchip', 'Mikroçip ID')}
                </div>
                <p className="text-xs font-extrabold text-slate-900 truncate">
                  {activeCat.microchipId || t('profile.notRegistered', 'Kayıtlı Değil')}
                </p>
              </div>

              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/60">
                <div className="flex items-center text-xs font-semibold text-slate-400 gap-1.5 mb-1">
                  <Heart className="w-3.5 h-3.5 text-rose-500" /> {t('profile.healthStatus', 'Sağlık Durumu')}
                </div>
                <p className="text-xs font-extrabold text-emerald-600">{t('profile.healthPerfect', 'Mükemmel (Aşılar Tam)')}</p>
              </div>
            </div>
          </section>
        )}
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">{t('profile.yourCatsSub', 'Profil kartına tıklayarak aktif kediyi değiştirebilirsiniz.')}</p>
            </div>
            <button
              type="button"
                ...
                >
                  {isActive && (
                    <span className="absolute top-3 right-3 px-2.5 py-0.5 bg-[#97480d] text-white text-[10px] font-black rounded-full uppercase tracking-wider">
                      {t('profile.activeProfile', 'Aktif Profil')}
                    </span>
                  )}
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-semibold text-slate-600">
                    <span>{cat.ageYears} {t('profile.years', 'Yıl')} {cat.ageMonths} {t('profile.months', 'Ay')}</span>
        ...
        <section className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm space-y-4">
          <div>
            <h3 className="text-lg font-black text-slate-900 tracking-tight font-serif">
              {t('profile.legalTitle', 'App Store & Yasal Haklar')}
            </h3>
            <p className="text-xs text-slate-500 font-medium">
              {t('profile.legalSub', 'Apple App Store şartları, gizlilik hakları ve satın alım yönetimi.')}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              ...
              <span>{t('profile.restorePurchases', 'Satın Alımları Geri Yükle')}</span>
            </button>
            ...
            >
              ...
              <span>{t('profile.rateApp', 'Uygulamayı Değerlendir ⭐')}</span>
            </button>
            ...
            >
              ...
              <span>{t('profile.exportData', 'Verilerimi İndir (GDPR JSON)')}</span>
            </button>
            ...
            >
              ...
              <span>{t('profile.deleteAccount', 'Hesabı ve Tüm Verileri Sil')}</span>
            </button>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 text-xs font-bold text-slate-500 pt-2 border-t border-slate-100">
            ...
            >
              <FileText className="w-3.5 h-3.5" /> {t('profile.termsOfUse', 'Kullanım Koşulları (EULA)')}
            </a>
            <span>•</span>
            <a
              ...
              className="hover:text-[#97480d] flex items-center gap-1 transition-colors"
            >
              <Shield className="w-3.5 h-3.5" /> {t('profile.privacyPolicy', 'Gizlilik Politikası')}
            </a>
          </div>
        </section>
      </main>
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs">
            ...
            >
              ...
              <h3 className="text-lg font-black text-slate-900 font-serif">{t('profile.deleteModalTitle', 'Hesabı ve Tüm Verileri Sil')}</h3>
              <p className="text-xs text-slate-600 font-medium">
                {t('profile.deleteModalDesc', 'Bu işlem geri alınamaz. Kedilerinizin tüm eğitimi, ağırlık geçmişi ve hatırlatıcı verileri cihazdan kalıcı olarak silinecektir.')}
              </p>
